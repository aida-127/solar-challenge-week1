# benin_eda.py - Solar Data Analysis for Benin
...
missing_percent = (missing_values / len(benin_df)) * 100
...
print(f"✅ Dropped 'Comments' column. New shape: {benin_clean.shape}")

# Convert timestamp
benin_clean['Timestamp'] = pd.to_datetime(benin_clean['Timestamp'])
benin_clean['Hour'] = benin_clean['Timestamp'].dt.hour
benin_clean['Month'] = benin_clean['Timestamp'].dt.month
print("✅ Converted timestamp and extracted time features")

# Outlier detection
key_columns = ['GHI', 'DNI', 'DHI', 'ModA', 'ModB', 'WS', 'WSgust']
z_scores = np.abs(stats.zscore(benin_clean[key_columns]))

print("\nOutliers (Z-score > 3):")
outlier_counts = (z_scores > 3).sum(axis=0)
for col, count in zip(key_columns, outlier_counts):
    print(f"  {col}: {count} outliers ({count/len(benin_clean)*100:.2f}%)")

# Remove extreme outliers
extreme_outliers = (z_scores > 5).any(axis=1)
benin_clean = benin_clean[~extreme_outliers]
print(f"✅ Removed {extreme_outliers.sum()} extreme outlier rows")
print(f"📈 Final cleaned shape: {benin_clean.shape}")
...
print(f"Data covers: {benin_clean['Timestamp'].min()} to {benin_clean['Timestamp'].max()}")
print(f"Total days: {(benin_clean['Timestamp'].max() - benin_clean['Timestamp'].min()).days}")

# Daily patterns
daytime_data = benin_clean[benin_clean['GHI'] > 0]
hourly_ghi = daytime_data.groupby('Hour')['GHI'].mean()

plt.figure(figsize=(10, 6))
plt.plot(hourly_ghi.index, hourly_ghi.values, marker='o', linewidth=2, color='orange')
plt.title('Average Solar Radiation (GHI) by Hour - Benin')
plt.xlabel('Hour of Day')
plt.ylabel('Average GHI (W/m²)')
plt.grid(True)
plt.savefig('benin_hourly_ghi.png')
plt.show()
...
plt.tight_layout()
plt.savefig('benin_correlation_heatmap.png')
plt.show()
...
print("✅ Cleaned data saved to: data/clean/benin_clean.csv")

print("\n🎉 BENIN ANALYSIS COMPLETED! 🎉")