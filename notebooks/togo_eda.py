# togo_eda.py - Solar Data Analysis for Togo
...
print("🚀 Starting Togo Solar Data Analysis...")

# Load data
togo_df = pd.read_csv('data/raw/togo-dapaong_qc.csv')
print("✅ Togo data loaded successfully!")
...
missing_percent = (missing_values / len(togo_df)) * 100
...
if 'Comments' in togo_df.columns and togo_df['Comments'].isna().all():
    togo_clean = togo_df.drop('Comments', axis=1)
    print(f"✅ Dropped 'Comments' column. New shape: {togo_clean.shape}")
else:
...
print(f"Data covers: {togo_clean['Timestamp'].min()} to {togo_clean['Timestamp'].max()}")
print(f"Total days: {(togo_clean['Timestamp'].max() - togo_clean['Timestamp'].min()).days}")

# Daily patterns
daytime_data = togo_clean[togo_clean['GHI'] > 0]
hourly_ghi = daytime_data.groupby('Hour')['GHI'].mean()

plt.figure(figsize=(10, 6))
plt.plot(hourly_ghi.index, hourly_ghi.values, marker='o', linewidth=2, color='red')
plt.title('Average Solar Radiation (GHI) by Hour - Togo')
plt.xlabel('Hour of Day')
plt.ylabel('Average GHI (W/m²)')
plt.grid(True)
plt.savefig('togo_hourly_ghi.png')
plt.show()
...
plt.title('Correlation Heatmap - Togo Solar Data')
plt.tight_layout()
...
print("✅ Cleaned data saved to: data/clean/togo_clean.csv")

print("\n🎉 TOGO ANALYSIS COMPLETED! 🎉")